...
class ChatUi:
        ...
        with gr.Blocks(title=TITLE, css=self._css()) as demo:
            ...
            with gr.Row(elem_classes=["app-row"]):
                ...
                with gr.Column(scale=3, elem_id="chat-pane"):
                    self._build_chat_area(selected_agent_id)
                with gr.Column(scale=1, min_width=230, elem_id="agent-params-pane"):
                    self._build_agent_params(selected_agent_id)
    ...
    def _build_agent_params(self, selected_agent_id: gr.State) -> None:
        """Правая панель: параметры выбранного агента."""
        gr.Markdown(AGENT_PARAMS_TITLE)
        self._agent_params = gr.Markdown(NO_AGENT_SELECTED_TEXT)

        def show_params(agent_id: Optional[str]) -> str:
            agent = self._registry.get(agent_id)
            if agent is None:
                return NO_AGENT_SELECTED_TEXT
            return f"**Модель LLM:** {agent.model}"

        selected_agent_id.change(
            fn=show_params,
            inputs=selected_agent_id,
            outputs=self._agent_params,
        )

    def _build_chat_area(self, selected_agent_id: gr.State) -> None:
    ...
    def _wire_selection_side_effects(self, selected_agent_id: gr.State) -> None:
        ...
        def show_agent_chat(agent_id: Optional[str]) -> Tuple:
            ...
            return agent.history, gr.update(visible=True), gr.update(visible=False)

        selected_agent_id.change(
            fn=show_agent_chat,
            inputs=selected_agent_id,
            outputs=[self._chatbot, self._chat_column, self._empty_chat_placeholder],
        )
    ...
    def _wire_chat_events(self, selected_agent_id: gr.State) -> None:
        ...
        self._send_button.click(
            ...
            outputs=[self._chatbot, self._message_input],
        )
    ...
    def _wire_new_agent_dialog(
        ...
        def create_agent(
            name: str, model: str, current_selected: Optional[str]
        ) -> Tuple:
            ...
            agent = self._registry.create(name=clean_name, model=model)
            return (
