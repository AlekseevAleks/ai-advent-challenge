...
class FakeAsyncClient:
    ...
    def chat_requests(cls) -> List[Dict[str, Any]]:
        """Only the chat completions, excluding memory-extraction calls."""
        return [
            request
            for request in cls.all_requests()
            if request["url"].endswith("/chat/completions")
            and "Memory Extractor"
            not in (request.get("json") or {}).get("messages", [{}])[0].get(
                "content", ""
            )
        ]
    ...
    class ConfigurableFakeClient(FakeAsyncClient):
        ...
        def _route(self, method: str, url: str, payload: Any = None) -> FakeResponse:
            ...
            if url.endswith("/chat/completions"):
                # The memory extractor uses its own system prompt; answer it
                # separately so chat tests are not affected by extraction.
