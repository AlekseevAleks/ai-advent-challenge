...
-- Task lifecycle states: the vocabulary of the state machine. The four
-- built-in stages are seeded on first use, but the user may add, rename,
-- deactivate or remove states, so the list is data, not code.
CREATE TABLE IF NOT EXISTS task_state_definitions (
    id          TEXT PRIMARY KEY,
    name        TEXT NOT NULL,
    description TEXT NOT NULL DEFAULT '',
...
-- Transition rules: which moves between states are allowed, and under which
-- structured conditions. Loaded from the database on every transition, so a
-- rule edited in the UI applies to the very next request.
...
CREATE INDEX IF NOT EXISTS idx_transition_rules_from
    ON transition_rules (from_state, active);

-- Transition history: every attempt, accepted or rejected, with its trigger
-- and reason. This is the audit trail of the state machine.
