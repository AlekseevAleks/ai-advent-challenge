...
class TaskStateManager:
    ...
    def create_state(
        ...
        *,
        ...
        ``stage`` overrides the default starting stage, which is how the
        configured initial state of the lifecycle is applied.
        """
        if self._repo.exists(task_id):
            return self.get_state(task_id)

        data = TaskStateData(
            stage=payload.stage if payload else (stage or "planning"),
    ...
    def update_step(
        ...
        *,
        ...
        ``metadata`` is merged by default, so a caller can add one fact without
        restating the rest. ``replace_metadata=True`` overwrites the whole map
        instead, which is what removing a fact needs.
        """
    ...
    def apply_step_suggestion(
        ...
        """Store the descriptive part of a suggestion produced by extraction.

        Only ``current_step`` and ``expected_action`` are applied here. The
        stage is deliberately **not** touched: moving the task is the job of
        :class:`~backend.transitions.manager.TransitionManager`, which checks
        the rules the user configured. The extractor's stage proposal is
        returned to the caller so it can be routed through that manager.
        """
        ...
        self._repo.save(task_id, state.chat_id, data)
        logger.info(
            "[TASK] Шаг пересчитан '%s': stage=%s step=%s expected=%s"
            " (confidence %.2f)",
            task_id,
            data.stage,
            data.current_step or "—",
    ...
    def transition(
        ...
        *,
        ...
        ``validated=True`` means the caller has already checked the move
        against the configured lifecycle (``TransitionManager`` does this), so
        the built-in edge list is not applied a second time. The default
        lifecycle and the configured one agree unless the user changed the
        rules, and in that case the user's rules win.
        """
        state = self.get_or_create(task_id)
        data = state.data
        current = data.stage
        ...
        if not validated and new_stage not in self.allowed_transitions(current):
