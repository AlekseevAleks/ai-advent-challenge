...
class FakeAsyncClient:
    ...
    #: How many background calls one exchange triggers at most: the transition
    #: gate (before the reply), memory extraction and the task step extraction.
    MAX_ANALYSIS_CALLS = 3

    @classmethod
    def wait_for_analysis(
        cls, expected: int = MAX_ANALYSIS_CALLS, timeout: float = 3.0
    ) -> None:
        """Block until at least ``expected`` analysis calls have been recorded.

        One exchange triggers up to three background calls: the transition gate
        (before the reply), memory extraction and the task step extraction. They
        run asynchronously, so tests must give them a moment to reach the
        (fake) API before asserting on the result.

        Waiting for the maximum is safe: the loop returns as soon as the count
        is reached, and a test that expects fewer calls simply waits for the
        full batch to settle.
        """
        import time
...
def fake_api(monkeypatch: pytest.MonkeyPatch):
    ...
    class ConfigurableFakeClient(FakeAsyncClient):
        ...
        #: Answer returned for the transition-detection prompt. Defaults to
        #: "no transition needed"; tests override it to propose a move.
        transition_response: FakeResponse = FakeResponse(
            200,
            {
                "choices": [
                    {
        ...
        #: Answer returned for the fact-extraction prompt. Defaults to "no
        #: facts in the message"; tests override it to supply values.
        fact_response: FakeResponse = FakeResponse(
            200,
            {
                "choices": [
                    {
        ...
        def _route(self, method: str, url: str, payload: Any = None) -> FakeResponse:
            ...
            if url.endswith("/chat/completions"):
                ...
                if "Transition Detector" in system_text:
                    return self.transition_response
                if "Fact Extractor" in system_text:
                    return self.fact_response
