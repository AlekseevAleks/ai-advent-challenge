...
def _propose_transition(fake_api, to_state: str, confidence: float = 0.9) -> None:
    """Make the pre-flight gate propose a transition for the request."""
...
def test_plan_approval_moves_to_execution(client: TestClient, fake_api) -> None:
    """The stage moves through the pre-flight gate, before the request runs."""
    _configure(client)
    chat = _create_chat(client)
    task_id = chat["id"]
    client.post(f"/api/tasks/{task_id}/state", json={})

    # The gate decides the transition for the request, before it is executed.
...
def test_stage_is_computed_in_the_same_call(client: TestClient, fake_api) -> None:
    ...
    The stage it returns is a proposal: the move itself is applied by the
    pre-flight gate, which checks it against the configured rules.
    """
    _configure(client)
    chat = _create_chat(client)
    task_id = chat["id"]
    ...
    state = client.get(f"/api/tasks/{task_id}/state").json()
    # The gate applied the move; the extractor supplied the step and action.
    assert state["data"]["stage"] == "execution"
    assert state["data"]["current_step"] == "implement authentication"
    assert state["data"]["expected_action"] == "create login endpoint"
...
def test_stage_only_suggestion_is_applied(client: TestClient, fake_api) -> None:
    ...
    The move comes from the pre-flight gate, which runs on the request itself.
    """
    _configure(client)
    chat = _create_chat(client)
    task_id = chat["id"]
...
def test_stage_jump_is_refused_by_the_rules(client: TestClient, fake_api) -> None:
    """A proposed jump is refused: the stage only moves through the rules.

    The extractor may propose ``done``, but ``planning → done`` has no rule, so
    the stage stays where it is. The step is still stored.
    """
    _configure(client)
    chat = _create_chat(client)
    task_id = chat["id"]
    ...
    state = client.get(f"/api/tasks/{task_id}/state").json()
    assert state["data"]["stage"] == "planning"  # not moved
    assert state["data"]["current_step"] == "finished"  # step still applied
...
def test_manager_applies_only_the_step_from_suggestion(database) -> None:
    """The step manager stores the step; the stage is the lifecycle's job."""
    from backend.tasks.manager import TaskStateManager
    from backend.tasks.models import TaskStepSuggestion
    ...
    assert state is not None
    # The stage is untouched: TransitionManager owns it.
    assert state.data.stage == "planning"
    assert state.data.current_step == "implement authentication"
    assert state.data.expected_action == "create login endpoint"


def test_manager_ignores_stage_jump(database) -> None:
    """A jump proposed by the extractor never moves the stage on its own."""
    from backend.tasks.manager import TaskStateManager
    from backend.tasks.models import TaskStepSuggestion
