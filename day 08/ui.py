...
class ChatUi:
    ...
    def _build_agent_params(self) -> None:
        """Правая панель: заполняется при выборе агента и после отправки сообщения."""
    ...
    def _format_params(self, agent: Agent) -> str:
        """Правая панель: модель, лимит контекста и расход токенов за диалог."""
    ...
    def _wire_chat_events(self, selected_agent_id: gr.State) -> None:
        def send_message(
            ...
            if not clean_text:
                yield agent.chat_messages, "", self._format_params(agent)
                return
        ...
        self._send_button.click(
            ...
            outputs=[self._chatbot, self._message_input, self._agent_params],
        )
