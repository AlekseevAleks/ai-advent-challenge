...
AGENT_STRATEGY_LABEL = "Стратегия управления контекстом"
...
PARAMS_FACTS_TITLE = "### Facts"
PARAMS_FACTS_UNSET = "Фактов пока нет: они появятся после сообщений."
PARAMS_BRANCHES_TITLE = "### Ветки диалога"
BRANCH_SWITCH_LABEL = "Переключить"
BRANCH_CREATE_LABEL = "Создать ветку"
BRANCH_SAVE_CHECKPOINT_LABEL = "Сохранить checkpoint"
BRANCH_NAME_PLACEHOLDER = "Имя ветки"
EMPTY_BRANCHES_TEXT = "Веток пока нет:checkpoint создаст первую ветку."
...
class ChatUi:
    ...
    def build(self) -> gr.Blocks:
        with gr.Blocks(title=TITLE, css=self._css()) as demo:
            ...
            self._wire_branch_events(selected_agent_id)
    ...
    def _build_dialogs(self) -> None:
        ...
        with self._new_agent_dialog:
            with gr.Column(elem_classes=["dialog-panel"]):
                ...
                self._agent_strategy_radio = gr.Radio(
                    label=AGENT_STRATEGY_LABEL,
    ...
    def _build_summary_box(self) -> None:
        """Правая панель: сводка, факты или ветки — по стратегии агента."""
        with gr.Column(elem_classes=["summary-box"], visible=True) as self._summary_box:
            gr.Markdown(PARAMS_SUMMARY_TITLE)
            self._agent_summary = gr.Markdown(PARAMS_SUMMARY_UNSET)
        with gr.Column(elem_classes=["facts-box"], visible=False) as self._facts_box:
            gr.Markdown(PARAMS_FACTS_TITLE)
            self._agent_facts = gr.Markdown(PARAMS_FACTS_UNSET)
        with gr.Column(elem_classes=["branch-box"], visible=False) as self._branch_box:
            gr.Markdown(PARAMS_BRANCHES_TITLE)
            self._branches_state = gr.State(None)
            self._branch_name_input = gr.Textbox(
                label=BRANCH_NAME_PLACEHOLDER,
                placeholder=BRANCH_NAME_PLACEHOLDER,
            )
            self._save_checkpoint_button = gr.Button(
                BRANCH_SAVE_CHECKPOINT_LABEL, size="sm"
            )
            self._branches_list = gr.Markdown(EMPTY_BRANCHES_TEXT)
            self._branch_switcher = gr.Radio(
    ...
    def _strategy_box_updates(self, agent: Optional[Agent]) -> Tuple:
        """Видимость правых панелей и их содержимое по стратегии агента."""
        strategy = agent.strategy if agent else None
        summary_visible = strategy == STRATEGY_SUMMARY
        facts_visible = strategy == STRATEGY_FACTS
        branches_visible = strategy == STRATEGY_BRANCHING
        summary_text = self._format_summary(agent) if summary_visible else ""
        facts_text = self._format_facts(agent) if facts_visible else ""
        branches_text = self._format_branches(agent) if branches_visible else ""
        active_branch_id = None
        branch_names = []
        if branches_visible and agent:
            for branch in agent.branches():
                branch_names.append(branch["name"])
                if branch["active"]:
                    active_branch_id = branch["id"]
        switcher_update = gr.update(
            choices=branch_names,
            value=None,
            visible=branches_visible and bool(branch_names),
        )
        return (
            gr.update(visible=summary_visible),
            gr.update(visible=facts_visible),
            gr.update(visible=branches_visible),
            summary_text,
            facts_text,
            branches_text,
            active_branch_id,
    ...
    def _format_summary(self, agent: Optional[Agent]) -> str:
        """Сводка старых сообщений или текст-заглушка."""
        summary = agent.summary_text() if agent else None
        return summary if summary else PARAMS_SUMMARY_UNSET

    def _format_facts(self, agent: Optional[Agent]) -> str:
        """Факты диалога списком или текст-заглушка."""
        facts = agent.facts_text() if agent else ""
        return facts if facts else PARAMS_FACTS_UNSET

    def _format_branches(self, agent: Optional[Agent]) -> str:
        """Список веток: активная помечается."""
        branches = agent.branches() if agent else []
        if not branches:
            return EMPTY_BRANCHES_TEXT
        lines = []
        for branch in branches:
            marker = "**" if branch["active"] else ""
            suffix = " (активна)" if branch["active"] else ""
            lines.append(f"- {marker}{branch['name']}{suffix}{marker}")
        return "\n".join(lines)
    ...
    def _wire_selection_side_effects(self, selected_agent_id: gr.State) -> None:
        ...
        def show_agent_chat(agent_id: Optional[str]) -> Tuple:
            ...
            box_updates = self._strategy_box_updates(agent)
            if agent is None:
                return (
                    [],
        ...
        selected_agent_id.change(
            ...
            outputs=[
                ...
                self._agent_facts,
                self._branches_list,
                self._branches_state,
                self._branch_switcher,
            ],
        )
        ...
        def send_message(
            ...
            if not clean_text:
                yield (
                    ...
                    "",
                    self._format_params(agent),
                    *self._strategy_box_updates(agent),
                )
                return
    ...
    def _wire_branch_events(self, selected_agent_id: gr.State) -> None:
        """Checkpoint создаёт ветку; переключение — по кнопке в списке."""

        def save_checkpoint(
            name: str, agent_id: Optional[str]
        ) -> Tuple:
            agent = self._registry.get(agent_id)
            if agent is None:
                raise gr.Error("Сначала выберите агента.")
            upto = len(agent.branch_history)
            branch = self._registry.create_branch(agent_id, upto)
            clean_name = name.strip()
            if clean_name and branch:
                agent._branches[branch["id"]]["name"] = clean_name
                if agent._store is not None:
                    agent._store.save_branch(
                        branch["id"],
                        agent.id,
                        agent._branches[branch["id"]]["parent_id"],
                        clean_name,
                        upto,
                    )
            return (
                self._registry.get(agent_id).chat_messages,
                "",
                *self._strategy_box_updates(agent),
            )

        self._save_checkpoint_button.click(
            fn=save_checkpoint,
            inputs=[self._branch_name_input, selected_agent_id],
            outputs=[
                self._chatbot,
                self._branch_name_input,
                self._summary_box,
                self._facts_box,
                self._branch_box,
                self._agent_summary,
        ...
        def switch_by_name(branch_name: Optional[str], agent_id: Optional[str]) -> Tuple:
            """Переключает ветку по имени из радио."""
            agent = self._registry.get(agent_id)
            if agent is None:
                raise gr.Error("Сначала выберите агента.")
            if branch_name:
                for branch in agent.branches():
                    if branch["name"] == branch_name:
                        self._registry.switch_branch(agent_id, branch["id"])
                        break
            updates = self._strategy_box_updates(agent)
            return (
                agent.chat_messages,
                updates[0],
                updates[1],
        ...
        self._branch_switcher.change(
            fn=switch_by_name,
            inputs=[self._branch_switcher, selected_agent_id],
            outputs=[
                self._chatbot,
                self._summary_box,
                self._facts_box,
    ...
    def _wire_new_agent_dialog(
        ...
        def toggle_recent_messages(strategy: str) -> dict:
            """Поле N видно для Summary и Sliding Window."""
            visible = strategy in (STRATEGY_SUMMARY, STRATEGY_SLIDING)
            return gr.update(visible=visible)

        self._agent_strategy_radio.change(
            fn=toggle_recent_messages,
            inputs=self._agent_strategy_radio,
            outputs=self._agent_recent_messages_input,
        )
