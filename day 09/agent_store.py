...
class AgentStore:
    ...
    def save_agent(
        ...
        with self._lock:
            self._connection.execute(
                "INSERT OR REPLACE INTO agents (id, name, model, context_limit, recent_messages_limit) "
                "VALUES (?, ?, ?, ?, ?)",
                (agent_id, name, model, context_limit, recent_messages_limit),
            )
            self._connection.commit()
    ...
    def _fetch_agents(self) -> list[dict]:
        cursor = self._connection.execute(
            "SELECT id, name, model, context_limit, recent_messages_limit, summary, "
            "summary_upto, summary_tokens FROM agents ORDER BY rowid"
        )
        return [
            {
                "id": row[0],
    ...
    def total_tokens_for_agent(self, agent_id: str) -> int:
        """Сумма total_tokens по сообщениям и суммаризациям агента."""
        with self._lock:
            ...
            summary_row = self._connection.execute(
                "SELECT COALESCE(summary_tokens, 0) FROM agents WHERE id = ?",
                (agent_id,),
            ).fetchone()
        return int(row[0]) + int(summary_row[0])
